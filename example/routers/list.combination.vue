<template>
    <hui-layout>
        ...
        <hui-navbar slot="navbar" title="Combination">
            <router-link to="/list" slot="left">
                <hui-navbar-back-icon></hui-navbar-back-icon>
            </router-link>
        </hui-navbar>

        <hui-pullrefresh :callback="loadList1" ref="prdemo">

            <hui-infinitescroll :callback="loadList2" ref="lsdemo">
                <hui-list theme="1" slot="list">
                    <hui-list-item v-for="item in list">
                        <img slot="img" :src="item.img">
                        <span slot="title">{{item.title}}</span>
                        <hui-list-other slot="other">
                            <div>
...
<script type="text/babel">
    export default {
        ...
        methods: {
            loadList1() {
                const url = 'http://list.hui.org/getdata.php';
                ...
                this.$http.jsonp(url, {params: {type: 'pulldown', page: this.page1}}).then((response) => {
                    setTimeout(() => {
                        ...
                        this.$refs.prdemo.$emit('hui.pullrefresh.finishLoad');
            ...
            loadList2() {
                this.$http.jsonp('http://list.hui.org/getdata.php?type=backposition', {
                    params: {
                        page: this.page2,
                    ...
                    setTimeout(() => { // 请求太快了，延迟一下方便看效果！
                        ...
                        if (_list.length < this.pageSize || this.page2 >= 4) {
                            // 所有数据加载完毕
                            this.$refs.lsdemo.$emit('hui.infinitescroll.loadedDone');
                            return;
                        }

                        // 单次请求数据完毕
                        this.$refs.lsdemo.$emit('hui.infinitescroll.finishLoad');
