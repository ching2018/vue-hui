<template>
    <div class="hui-checklist" :style="{color: color}" :class="align == 'right' ? 'hui-checklist-alignright' : ''">
        <slot></slot>
    </div>
</template>
...
<script type="text/babel">
    ...
    export default {
        ...
        methods: {
            checkItem() {
                const childrens = this.$children.filter(item => item.$options.name === 'hui-checklist-item');
        ...
        mounted() {
            this.$on('hui.checklist.checkall', this.checkAll);
        }
    }
</script>
