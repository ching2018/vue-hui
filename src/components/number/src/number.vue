<template>
    <span class="m-number" :style="{height: height, width: width}">
        <a href="javascript:;" ref="minus"></a>
        <input type="text" ref="numInput" class="number-input" :readonly="readonly" v-model="counter" placeholder=""/>
    ...
    export default {
        ...
        methods: {
            ...
            checkParameters() {
                ...
                if (max < unit && max != 0) {
                    console.error('[HUI warn]: The parameter \'max\'(' + max + ') must be greater than or equal to \'unit\'(' + unit + ').');
                    return false;
                }

                if (max % unit != 0) {
                    console.error('[HUI warn]: The parameter \'max\'(' + max + ') and \'unit\'(' + unit + ') must be multiple.');
                    return false;
                }

                if (min % unit != 0 && min >= 0) {
                    console.error('[HUI warn]: The parameter \'min\'(' + min + ') and \'unit\'(' + unit + ') must be multiple.');
                    return false;
                }

                if (max < min && max != 0) {
                    console.error('[HUI warn]: The parameter \'max\'(' + max + ') must be greater than to \'min\'(' + min + ').');
                    return false;
                }
...
<style lang="less">
    @import "../../../styles/components/number.less";
</style>
