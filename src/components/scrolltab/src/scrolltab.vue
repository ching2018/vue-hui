<template>
    <div class="hui-scrolltab">
        <div class="hui-scrolltab-nav" ref="navbox">
            <a href="javascript:;"
               class="hui-scrolltab-item"
               v-for="item, key in navList"
               :key="key"
               :ref="`tabitem_${key}`"
               :class="activeIndex == key ? 'hui-scrolltab-active' : ''"
               @click="moveHandler(key)">
                <div class="hui-scrolltab-icon"><i :class="item.icon"></i></div>
                <div class="hui-scrolltab-title">{{item.label}}</div>
            </a>
        </div>
        <div class="hui-scrolltab-content" ref="scrollView">
            <slot></slot>
        </div>
    </div>
</template>
...
<script type="text/babel">
    export default {
        ...
        methods: {
            ...
            getPanels() {
                return this.$children.filter(item => item.$options.name === 'hui-scrolltab-panel');
            },
            bindEvent() {
